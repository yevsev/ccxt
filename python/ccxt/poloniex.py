...
class poloniex(Exchange):
    ...
    def _websocket_on_message(self, contextId, data):
        ...
        if channelIdStr in symbolsIds:
            ...
            # Some error occured
            self.emit('err', new ExchangeError(self.id + '._websocketOnMessage() failed to get symbol for channelId: ' + channelIdStr))
    ...
    def _websocket_handle_orders(self, contextId, data):
        ...
        for i in range(0, len(datareceived)):
            ...
            if msg[0] == 'b':
                ...
                self.emit('err', new ExchangeError('Message Type ' + msg[0] + ' is not handle at the moment'))
                ...
                for i in range(0, len(orderbook)):
                    ...
                    if order[0] == 'o':
                        ...
                        if order[1] == 0:
                            ...
                            # error
                            self.emit('err', new ExchangeError(self.id + '._websocketHandleOb() unknown value in buy/sell field. Expected 0 or 1 but got: ' + order[1]))
                            self.websocketClose(contextId)
                            return
                    elif order[0] == 't':
                        ...
                        # unknown value
                        self.emit('err', new ExchangeError(self.id + '._websocketHandleOb() unknown value in order/trade field. Expected \'o\' or \'t\' but got: ' + order[0]))
                        self.websocketClose(contextId)
                        return
                if not self._contextIsSubscribed(contextId, 'ob', symbol):
                    return
