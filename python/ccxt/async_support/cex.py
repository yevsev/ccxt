...
class cex(Exchange):
    ...
    def _websocket_handle_auth(self, contextId, msg, oid, resData):
        ...
        if msg['ok'] == 'ok':
            ...
            self.emit('auth', False, new AuthenticationError(self.safe_string(resData, 'error', 'auth error')))
    ...
    def _websocket_handle_ob_update(self, contextId, msg, oid, resData):
        ...
        if data['ob']['nonce'] != (resData['id'] - 1):
            self.websocketClose()
            self.emit('err', new NetworkError('invalid orderbook sequence in ' + self.id + ' ' + data['ob']['nonce'] + ' != ' + resData['id'] + ' -1'))
        else:
